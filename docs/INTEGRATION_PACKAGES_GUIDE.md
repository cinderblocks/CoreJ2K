...
Each integration package registers an `IImageCreator` with `CoreJ2K.Util.ImageFactory` so the core library can convert to and from that package's image types (`img.As<SKBitmap>()`, `J2kImage.ToBytes(bitmap)`, etc.). How registration happens depends on the runtime:

- **JIT runtimes (.NET Framework, and .NET 8+ running normally)** — registration is automatic. On first use, `ImageFactory` scans the application directory for `CoreJ2K.*.dll` plugin assemblies and registers every creator it finds. Installing a package is enough, even if your code never references its types directly.
- **.NET 8+ plugin assemblies** — each package also self-registers via a `[ModuleInitializer]` that runs the first time anything in that assembly is used (for example one of its extension methods).
- **NativeAOT and trimmed apps** — dynamic discovery is unavailable, and a plugin assembly the compiler trims away cannot self-register. If a conversion fails with `No image creator registered for target type ...`, register explicitly at startup:

  ```csharp
  CoreJ2K.Util.ImageFactory.Register(new SKBitmapImageCreator());
  CoreJ2K.Util.ImageFactory.Register(new SKPixmapImageCreator());
  ```

  Registration is idempotent per image type, so calling `Register` manually on top of automatic discovery is harmless.
